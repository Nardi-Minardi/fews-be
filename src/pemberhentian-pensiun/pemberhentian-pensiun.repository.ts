...
export type PpnsPensiunUpdateInputWithExtra = Prisma.PpnsPemberhentianPensiunUpdateInput & {
  id_data_ppns?: number;
  ...
  tgl_sk_bkn?: Date | null;
};

@Injectable()
export class PemberhentianPensiunRepository {
  constructor(
  ...
  async savePpnsPensiun(
    id: number | null,
    data: PpnsPensiunUpdateInputWithExtra,
  ): Promise<CreateResponsePensiunPpnsDto> {
    ...
    let result;
    if (id) {
      result = await this.prismaService.ppnsPemberhentianPensiun.update({
        where: { id },
        data: {
          ...
          no_sk_bkn: data.no_sk_bkn,
          tgl_sk_bkn: data.tgl_sk_bkn,
        },
      });
    } else {
      result = await this.prismaService.ppnsPemberhentianPensiun.create({
        data: {
          id_data_ppns: data.id_data_ppns,
          id_surat: idSurat ?? undefined,
          ...
          no_sk_bkn: data.no_sk_bkn ?? undefined,
          tgl_sk_bkn: data.tgl_sk_bkn ?? undefined,
        },
      });
    }
    ...
    return {
      ...
      sk_bkn: {
        no_sk_bkn: result.no_sk_bkn ?? null,
        tgl_sk_bkn: result.tgl_sk_bkn
      },
    };
  }

  async findPpnsPensiunByIdDataPpns(id_data_ppns: number) {
    return this.prismaService.ppnsPemberhentianPensiun.findFirst({
